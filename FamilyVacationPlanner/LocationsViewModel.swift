...
@Observable class LocationsViewModel {
    ...
    var daySegmentsForFunction: [Segment] = [Segment(dayDate: Date(), dayString: "", startLocation: Location(), endLocation: Location())]
    ...
    func setUpDailySegments(trip: Trip) {
        ...
        daySegments.append(Segment(dayDate: dateOfDay, dayString: formattedDateString, startLocation: dayStartLocation , endLocation: dayEndLocation ))
        tripSegments[dayNumber].segments = daySegments
            ...
            for location in locations {
                ...
                if location.dateArrive ?? Date() >= dateOfDay && location.dateArrive ?? Date() < (dateOfDay + 3600 * 24) {
                    if !location.overNightStop && !location.startLocation {
                        let newSegment = Segment(dayDate: dateOfDay, dayString: formattedDateString, startLocation: daySegments[Int(location.startIndex)].startLocation, endLocation: location)
